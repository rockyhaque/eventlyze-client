...
export const updatedUserStatus = async (id: string, data:any) => {
    try {
        const response = await app_axios.patch(`/user/status/${id}`, {status: data});
        return response.data;
    } catch (error: any) {
        const message = error?.response?.data?.message || "Something went wrong while updating user status!";
        return new Error(message);
    }
};
export const updatedUserRole = async (id: string, data:any) => {
    try {
        const response = await app_axios.patch(`/user/update-role/${id}`, {role: data});
        return response.data;
    } catch (error: any) {
        console.log("error while fetching user", error)
        const message = error?.response?.data?.message || "Something went wrong while updating user role!";
        return new Error(message);
    }
};
